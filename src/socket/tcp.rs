...
impl<'a> Socket<'a> {
    ...
    pub fn process(
        ...
        match (self.state, control) {
            ...
            // Check `listen_endpoint` to ensure SYN-RECEIVED was converted from the LISTEN state
            // (another possible reason is TCP simultaneous open).
    ...
    ///
    /// Note that the RFC above specifies "at least 2*RMSS bytes". In practice, we follow the Linux
    /// kernel's empirical value of sending an ACK for every RMSS byte of new data. For details,
    /// see <https://elixir.bootlin.com/linux/v6.11.4/source/net/ipv4/tcp_input.c#L5747>.
